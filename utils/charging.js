...
function startCharging(stationId, pileId, options) {
  ...
  const orderId = `od-${startTime}-${Math.floor(Math.random() * 1000)}`;
  const session = {
    orderId,
    ...
    startSoc: typeof opts.startSoc === 'number' ? opts.startSoc : 32,
...
function computeProgress(session, now) {
  ...
  const startSoc = typeof session.startSoc === 'number' ? session.startSoc : 32;
...
function payOrder(orderId, method, coupon) {
  ...
  const couponAmount = coupon ? Math.min(Number(coupon.amount) || 0, order.totalCost) : 0;
  const payAmount = +Math.max(0, order.totalCost - couponAmount).toFixed(2);

  let result;
  if (method === 'balance') {
    result = storage.payByBalance(payAmount, `${order.stationName} 充电费`);
  } else {
    result = storage.payByWechat(payAmount, `${order.stationName} 充电费`);
  }
  if (!result.ok) return { ok: false, reason: result.reason, balance: result.balance };

  if (coupon) storage.consumeCoupon(coupon.id);
  ...
  const paid = storage.updateOrder(orderId, {
    ...
    payAmount,
    payMethod: method === 'balance' ? '余额支付' : '微信支付',
    paidAt: Date.now()
  });
